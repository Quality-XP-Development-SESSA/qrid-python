...
3. In that same run, `release.yml` dispatches [`publish.yml`](.github/workflows/publish.yml) via the GitHub API (`gh workflow run publish.yml`), which re-runs the tests, builds the sdist/wheel, and uploads to PyPI using [Trusted Publishing (OIDC)](https://docs.pypi.org/trusted-publishers/) — no stored API token.
